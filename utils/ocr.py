...
def preprocess_image(image_path):
    ...
    try:
        ...
        rotated = cv2.warpAffine(denoised, rot_mat, denoised.shape[1::-1], flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_REPLICATE)
...
def process_image(image_path):
    ...
    try:
        ...
        # Gemini APIで補完
        gemini_info = use_gemini_api(image_path)
        if gemini_info:
            info = combine_results(info, gemini_info)
...
def process_pdf(pdf_path):
    ...
    try:
        ...
        # 最も信頼度の高い結果を返す
        best_result = max(results, key=lambda x: len(x.get("支払先名", "")) + len(x.get("金額", "")))
        return best_result
...
def combine_results(ocr_result, gemini_result):
    ...
    for field in final_result.keys():
        ...
        # 金額の場合、数値のみを抽出して比較
        if field == '金額':
            ocr_amount = int(re.sub(r'[^\d]', '', ocr_value)) if re.sub(r'[^\d]', '', ocr_value).isdigit() else 0
            gemini_amount = int(re.sub(r'[^\d]', '', gemini_value)) if re.sub(r'[^\d]', '', gemini_value).isdigit() else 0
        ...
        # インボイス番号の場合、形式をチェック
            ...
            if re.match(r'T\d{13}', ocr_invoice):
                ...
                final_result[field] = ocr_invoice if len(ocr_invoice) > len(gemini_invoice) else gemini_invoice
...
def extract_info_from_text(text):
    ...
    amount_candidates = []
    amount_patterns = [
        (r'(?:領収金額|お支払金額|ご利用金額|合計金額|利用金額|お会計|小計)[\s:：]*[¥\\]?[\s]*([0-9,\.]+)', 3),  # 重み付け
        (r'(?:金額)[\s:：]*[¥\\]?[\s]*([0-9,\.]+)', 2),
        (r'[¥\\][\s]*([0-9,\.]+)', 1),
        (r'([0-9]+[\s\.]+[0-9]+)', 1),
...
def process_ocr_result(ocr_text, confidence_threshold=60):
    ...
    try:
        # 信頼度の高いテキストのみを使用
        filtered_text = "\n".join([
            word for word in ocr_text.split('\n')
